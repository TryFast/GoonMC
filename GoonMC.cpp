...
static Config load_config(const fs::path& p) {
    ...
    if (j.has("java_args"))   c.java_args   = j["java_args"].str();
...
static void save_config(const Config& c, const fs::path& p) {
    ...
    int n = snprintf(buf, sizeof(buf),
        "{\n  \"username\": \"%s\",\n  \"java_path\": \"%s\",\n  \"java_args\": \"%s\",\n  \"ram_gb\": %d,\n  \"theme_color\": %d\n}\n",
        esc_json(c.username).c_str(), esc_json(c.java_path).c_str(),
        esc_json(c.java_args).c_str(), c.ram_gb, c.theme_color);
...
static bool launch_version(const fs::path& root, const Config& cfg, const std::string& version) {
    ...
    if (!cfg.java_args.empty()) {
        std::string tok;
        std::istringstream ss(cfg.java_args);
...
static void section_settings(Config& cfg, const fs::path& cfg_path) {
    for (;;) {
        ...
        if (input == "1") {
            ...
            printf("Extra Java args (space-separated, e.g. -XX:+UseG1GC) [%s]: ",
                   cfg.java_args.empty() ? "none" : cfg.java_args.c_str());
            std::string val; std::getline(std::cin, val);
            cfg.java_args = val;
...
static void section_themes(Config& cfg, const fs::path& cfg_path) {
...
int main() {
    ...
    for (;;) {
        ...
        else if (input == "4") section_themes(cfg, cfg_path);
        else if (input == "5" || input == "q" || input == "Q") break;
    }
    return 0;
}